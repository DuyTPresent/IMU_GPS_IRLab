...
#fused_timestamps, fused_x, fused_y = read_csv_file('/home/d/ros_irl_cpp/src/eskf_localization/state.csv')
fused_timestamps, fused_x, fused_y = read_csv_file('/home/d/ros_irl_cpp/src/eskf_localization/error_1.csv')
# Vẽ đồ thị
plt.figure(figsize=(10, 6))

# Vẽ dữ liệu từ GPS
# plt.plot(gps_x, gps_y, label='GPS Data', color='red', linestyle='-')

# Vẽ dữ liệu đã fusion
plt.plot(fused_x, fused_y, label='Fused GPS+IMU Data', color='blue', linestyle='-')

# Đặt tiêu đề và nhãn trục
plt.title('GPS vs. Fused Data')
plt.xlabel('X Value')
plt.ylabel('Y Value')
